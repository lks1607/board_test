...
-----------------------------------------------------------------------------*/
window.IBS_CopySheet2Form = function(param) {
    var sheetobj,
        formobj,
        row,
        sheetPreFix,
    ...
    if ((!param.sheet) || (typeof param.sheet.version != "function")) {
        alert("IBS_CopyForm2Sheet 함수의 sheet 인자가 없거나 ibsheet객체가 아닙니다.");
        return false;
    }

    if (param.form == null || typeof param.form != "object" || param.form.tagName != "FORM") {
        alert("IBS_CopyForm2Sheet 함수의 form 인자가 없거나 FORM 객체가 아닙니다.");
        return false;
    }
    ...
    for (col = 0; col < cols.length ; col++) {
        ...
        sheetvalue = sheetobj.getString(row, sheetPreFix + baseName);
        ...
        switch (sType) {
            ...
            case "submit":
                break;
            case "select-one":
                frmchild.value = sheetobj.getValue(row, sheetPreFix + baseName);
                break;
            case "radio":
			sheetvalue = sheetobj.getValue(row, sheetPreFix + baseName);
                ...
                break;
            case "checkbox":
                frmchild.checked = (sheetobj.getValue(row, sheetPreFix + baseName) == 1);
                break;
            default:
